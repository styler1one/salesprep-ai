  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (error) {
        ...
        notifyOpener({ type: 'google_calendar_error', error })
        return
      }
      ...
      if (!code || !state) {
        ...
        notifyOpener({ type: 'google_calendar_error', error: 'Missing parameters' })
        return
      }

      // Verify state matches (optional security check)
      const storedState = sessionStorage.getItem('google_calendar_state')
      if (storedState && state !== storedState) {
        setStatus('error')
        setMessage('Invalid state parameter')
        notifyOpener({ type: 'google_calendar_error', error: 'State mismatch' })
        return
      }
      ...
      try {
        ...
        const { data, error: apiError } = await api.post<{ 
          ...
          email?: string
          provider: string 
        }>('/api/v1/calendar/callback/google', { code, state })

        if (apiError || !data?.success) {
          throw new Error(apiError?.message || 'Failed to connect calendar')
        }
